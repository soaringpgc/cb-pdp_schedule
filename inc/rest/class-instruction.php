...
class Instruction extends \Cloud_Base_Rest {
	...
	public function pdp_get_instruction ( \WP_REST_Request $request) {
		...
		if(isset($request['start'])){			
			$sql = $wpdb->prepare("SELECT * FROM {$table_name}  WHERE request_date = %d" , $request['start']);
...
//  create new instruction request 
	public function pdp_post_instruction ( \WP_REST_Request $request) {
		...
		if(isset($request['request_date']) ){	
			...
			if( !isset($request['scheduling_assistance'])){
				$display_name = $user_meta->first_name .' '.  $user_meta->last_name;
				if(isset($request['prim_instructor']) ){
					$inst = get_user_by('ID', $request['prim_instructor'] );
					$inst_meta = get_userdata( $user->ID );
					$display_inst = $user_meta->first_name .' '.  $user_meta->last_name;
				}
				if(isset($request['alt_instructor']) ){
					$alt_inst = get_user_by('ID', $request['alt_instructor'] );
					$alt_inst_meta = get_userdata( $user->ID );
					$display_alt_inst = $user_meta->first_name .' '.  $user_meta->last_name;
				}
			}
			isset($request['inst_type']) ? $inst_type = $request['inst_type'] : $inst_type = 1 ;
			isset($request['comment']) ? $comment = $request['comment'] : $comment ="";
			isset($request['confirmed']) ? $confirmed = true : $confirmed = false;
			isset($request['scheduling_assistance']) ? $assistance = true : $assistance = false ;
...
// 			$result = $wpdb->insert($table_name , $record);		 						
			...
			if ( $assistance ) {
				...
				$to = $inst_meta->user_email . ', ' .  $alt_inst_meta->user_email . ', ' . $user_meta->user_email ; 		
			}
	...
	var_dump($msg, $to, $subject, $assigned_time );
	die();	
